function C2task56_shortcircuit_node6_ode45()
...
%% 1. 运行潮流计算获取初始状态
fprintf('=== 电力系统稳定分析 (节点6三相短路 - ode45版本) ===\n');
...
% 计算故障期间的转移阻抗（节点6三相短路）
X_transfer_fault = calculate_transfer_impedance_fault_node6_star_delta();
fprintf('  故障期间转移阻抗:X_transfer_fault = %.6f pu\n', X_transfer_fault);

% 计算故障后转移阻抗（切除线路3-6的其中一回）
X_transfer_post = calculate_transfer_impedance_post_node6_star_delta();
...
% 绘制功角和角速度曲线
plot_stability_results(t, delta_deg, omega_rpm, fault_start, fault_cleared, '节点6三相短路');
...
function X_transfer = calculate_transfer_impedance_star_delta()
    ...
    fprintf('      总转移阻抗: Z_18 = j%.4f\n', imag(Z_18_total));
    X_transfer = abs(imag(Z_18_total));
end

%% 星三角变换方法计算转移阻抗（节点6三相短路故障期间）
function X_transfer_fault = calculate_transfer_impedance_fault_node6_star_delta()
    fprintf('  使用网络变换法计算节点6三相短路故障期间转移阻抗...\n');
    ...
    % 节点6三相短路，相当于节点6接地
    % 发电机到无穷大系统的路径被中断，转移阻抗非常大
    % 使用星三角变换计算
    
    % 步骤1: 将节点6接地，计算从发电机到无穷大系统的等效阻抗
    % 由于节点6接地，从发电机到无穷大系统没有直接路径
    % 转移阻抗主要由发电机阻抗和变压器阻抗决定
    ...
    % 节点6接地，所以从节点8到无穷大系统节点1的阻抗为无穷大
    % 但实际计算中，我们用一个很大的值表示
    
    fprintf('    节点6三相短路，发电机与系统解列，转移阻抗很大\n');
    X_transfer_fault = 1e6; % 用一个很大的值表示解列状态
end

%% 星三角变换方法计算转移阻抗（节点6短路故障后）
function X_transfer_post = calculate_transfer_impedance_post_node6_star_delta()
    fprintf('  使用网络变换法计算节点6短路故障后转移阻抗...\n');
...
% 绘制功角曲线和转速曲线
function plot_stability_results(t, delta_deg, omega_rpm, fault_start, fault_cleared, fault_type)
    ...
    xlabel('时间 (s)');
    ylabel('功角 (度)');
    title(['发电机功角摇摆曲线 - ' fault_type ' (ode45)']);
    ...
    xlabel('时间 (s)');
    ylabel('转速 (rpm)');
    title(['发电机转速变化曲线 - ' fault_type ' (ode45)']);
